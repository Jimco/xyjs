/**
 * xyAnimate Animate framework
 * Date: 2012-10-19
 *
 */

var xyAnimate = function(target) {
  ...
  return new xyAnimate.pt.init(collection);
};
...
xyAnimate.pt = xyAnimate.prototype = {
  callbacks: {},
  collection: [],
  counter: 0,
  p: 0,
  ...
  init: function(collection) {
    var div, style = "", props = ["moz", "o", "webkit", "ms"], 
        pre = xyAnimate.config.pre, b = xyAnimate.config.bro, self = this, transition = self.t;
  ...
  _animate: function(idx, opts) {
    ...
    if (typeof opts.duration === self.u) {
      opts.duration = xyAnimate.config.duration;
    }
    if (typeof opts.delay === self.u) {
      opts.delay = xyAnimate.config.delay;
    }
    if (typeof opts.easing === self.u) {
      opts.easing = xyAnimate.config.easing;
    }
  ...
  maxDuration: function(animations) {
    ...
    animations.forEach(function(el) {
      if (typeof el.duration === self.u) {
        el.duration = xyAnimate.config.duration;
      }
      if (typeof el.delay === self.u) {
        el.delay = xyAnimate.config.delay;
      }
...
xyAnimate.pt.init.prototype = xyAnimate.pt;
